...
async def topics_command(update: Update, context) -> None:
    ...
    try:
        ...
        await update.message.reply_text("获取话题信息失败，请稍后重试。")
...
async def select_command(update: Update, context) -> None:
    ...
    try:
        ...
        await update.message.reply_text("选择话题失败，请稍后重试。")
...
async def handle_file_upload(update: Update, context, file_type: str) -> None:
    ...
    try:
        # 发送处理中消息
        processing_msg = await message.reply_text(f"正在处理{file_type}...")
        ...
        if result['success']:
            ...
            await processing_msg.edit_text(f"处理{file_type}失败：{result['error']}")
            
    except Exception as e:
        logger.error(f"处理文件上传时发生错误: {str(e)}")
        try:
            await message.reply_text("处理文件时发生错误，请稍后重试。")
        except:
            pass
